    ...
    * Check that atleast one file is staged
    ...
    with working_area(included) as area:
        ...
        with ProgressBar(_("Preparing")) as progress_bar:
            ...
            # Git add all included files in batches
            def batch_files(files, size=100):
...
def _rank_similar_slugs(target_slug, other_slugs):
    ...
    for other_slug in other_slugs:
        scores[other_slug] = jellyfish.jaro_winkler_similarity(target_slug, other_slug)
...
class Slug:
    ...
    def _get_branches(self):
        ...
        if self.offline:
            ...
            try:
                with spawn(cmd, timeout=10) as child:
